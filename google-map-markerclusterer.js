 ...
 */
class GoogleMapMarkerclusterer extends GoogleMapOverlayviewMixin(PolymerElement) {
  static get template() {
    return html`
  ...
  ready() {
    super.ready();
    setTimeout(this._initClusterIconTemplate.bind(this));
  ...
  */
  ...
  onAdd() {
    var mc = this;
    this.isReady = true;
    this._repaintDebouncer = Debouncer.debounce(
      this._repaintDebouncer,
      timeOut.after(250),
  ...
  */
  _computeClusters(iFirst) {
    ...
    if (!this.isReady || !this.markers) return;

    // Cancel previous batch processing if we're working on the first batch:
    if (iFirst === 0) {
      this.dispatchEvent(new CustomEvent(
        'google-map-markercluster-clustering-start',
    ...
    if (iLast < this.markers.length) {
      ...
      delete this.timerRefStatic;
      this.dispatchEvent(new CustomEvent(
        'google-map-markercluster-clustering-end',
        { bubbles: true, composed: true }));
    }
  }
  ...
  _changeStyles(styles) {
    if (!this._clusterIconTemplate) return;

    const defaultIcon = this._clusterIconTemplate.querySelector(
      'google-map-defaulticon');
...
window.customElements.define(GoogleMapMarkerclusterer.is, GoogleMapMarkerclusterer);

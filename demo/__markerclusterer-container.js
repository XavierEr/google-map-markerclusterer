...
class MarkerclustererContainer extends PolymerElement {
  static get template() {
    return html`
    <style>
      #map {
  ...
  ready() {
    ...
    setTimeout(function() {
      if (this.$.custom_cluster_icon.assignedNodes().length > 0) {
        this._isStyleVisible = false;
      }
    }.bind(this));
